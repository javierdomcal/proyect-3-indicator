...
class ClusterConnection:
    ...
    def load_config(self):
        ...
        with open(self.config_file, "r") as f:
            ...
            if not config:
                raise ValueError(
                    f"Cluster '{self.cluster_name}' configuration not found."
                )
            ...
            if not self.hostname or not self.username:
                raise ValueError(
                    "Hostname and username must be provided for the selected cluster."
                )
    ...
    def transfer_between_clusters(
        source_conn, target_conn, remote_source_path, remote_target_path
    ):
        """
        ...
        os.remove(temp_path)
        print(
            f"Transferred {remote_source_path} from {source_conn.hostname} to {remote_target_path} on {target_conn.hostname}."
        )


if __name__ == "__main__":
    # Load configurations for different clusters
    atlas_connection = ClusterConnection(
        config_file="utils/cluster_config.json", cluster_name="atlas"
    )
    llum_connection = ClusterConnection(
    ...
    try:
        ...
        atlas_connection.download_file(
            "remote_path/result1.txt", "local_path/result1.txt"
        )

        # Connect to the second cluster for further tasks
        llum_connection.connect()
        llum_connection.upload_file(
            "local_path/result1.txt", "another_remote_path/result1.txt"
        )
