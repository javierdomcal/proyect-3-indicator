        ...
        while True:
            try:
                ...
                if job_id not in output:
                    ...
                    logging.info(f"Job {job_id} for {full_name} completed after {runtime}")
                    return "COMPLETED"

                # Job still running
                logging.info(f"Job {job_id} for {full_name} still running, checking again in {delay} seconds")
                time.sleep(delay)
...
if __name__ == "__main__":
    ...
    try:
        with ClusterConnection() as connection:
            ...
            output = commands.execute_command(f"sbatch --wrap='sleep 10' --job-name={test_name}")
