...
class DMNCalculation(Calculation):
    def prepare_input_files(self, job_name):
        ...
        try:
            ...
            colony_log_path = os.path.join(self.connection.colony_dir, job_name, log_file)
            ...
            self.file_manager.upload_file(
                slurm_script,
                os.path.join(self.connection.colony_dir, job_name, f"{job_name}_dmn.slurm")
            )
        ...
        try:
            ...
            for file in required_files:
                ...
                if not self.commands.check_file_exists(file_path):
                    raise FileNotFoundError(f"Required DMN output file {file} not found")
