...
class InputFileGenerator:
    ...
    def _generate_gaussian_input(self, nproc, wfx=True):
        ...
        basis_type = "gen " if self.inp.basis.is_even_tempered or self.inp.basis.is_imported else f"{self.inp.basis.basis_name} "
        ...
        with open(filename, 'w') as f:
            ...
            # Gaussian options
            f.write('gfinput fchk=all '+ self.inp.method.method_keywords + wfx_text + "\n\n")

            # Title line
            molecule_desc = self.inp.molecule.get_molecule_description()
            f.write(f"{self.title} {molecule_desc} {self.inp.method.method_name} {self.inp.basis.basis_name}\n\n")
            ...
            if self.inp.molecule.is_harmonium:
                f.write("    0.0000000000D+00    0.0000000000D+00    0.0000000000D+00\n")
                f.write(f"   {-self.inp.molecule.omega**2 / 2:.10f}   {-self.inp.molecule.omega**2 / 2:.10f}   {-self.inp.molecule.omega**2 / 2:.10f}\n")
                for _ in range(9):
                    f.write("    0.0000000000D+00    0.0000000000D+00    0.0000000000D+00\n")
    ...
    def _write_basis_coefficients(self, file_handle, atom):
        ...
        print(f"Basis coefficients for {atom} from {basis_file_path} written successfully.")
